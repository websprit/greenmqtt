...
async fn mqtt_v5_ws_pending_inbound_qos2_limit_disconnects_quota_exceeded() {
    ...
    client
        .send(Message::Binary(
            (connect_packet_v5("qos2-budget-ws")).into(),
        ))
        .await
        .unwrap();
    ...
    for packet_id in 1..=crate::mqtt::session::MAX_PENDING_INBOUND_QOS2 as u16 {
        client
            .send(Message::Binary(
                (publish_packet_v5_qos2(packet_id, "devices/d1/state", b"budget-test")).into(),
            ))
            .await
            .unwrap();
...
async fn mqtt_ws_connect_subscribe_publish_flow() {
    ...
    publisher
        .send(Message::Binary(
            (publish_packet("devices/d1/state", b"ws-up")).into(),
        ))
        .await
        .unwrap();
...
async fn mqtt_publish_before_connect_closes_websocket() {
    ...
    client
        .send(Message::Binary(
            (publish_packet("devices/d1/state", b"up")).into(),
        ))
        .await
        .unwrap();
...
async fn mqtt_v5_ws_invalid_subscription_qos_disconnect_includes_server_reference() {
    ...
    client
        .send(Message::Binary(
            (connect_packet_v5("invalid-subscription-qos-ws-ref")).into(),
        ))
        .await
        .unwrap();
    let connack = client.next().await.unwrap().unwrap().into_data();
